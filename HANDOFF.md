...
13. **큰 케이스에서 애니메이션 프리로드가 메모리 부족으로 크래시하던 문제** (`animation_bar.py`) —
    `_anim_cache`에 상한이 없어서 프레임을 볼 때마다/프리로드할 때마다 무제한으로 쌓임. 입자 수가
    많은 케이스(17900개/프레임, 2250프레임)로 직접 재현: 프로세스 메모리가 45초 만에 21GB를 넘기며
    계속 증가(전체 캐싱하려면 약 500GB 필요) → OOM 크래시로 이어짐. `psutil.virtual_memory()`로
